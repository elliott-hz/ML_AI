 ...
 * A binary function has three rates of change:
 *   • rate w.r.t x  (hold y constant)
 *   • rate w.r.t y  (hold x constant)
 *   • rate w.r.t both x and y (simultaneous)
 * This page motivates the need to isolate individual rates.
 */
  ...
  return (
    <PageContainer>
      ...
      <SectionDescription>
        ...
        Here <strong>z = x² + y²</strong> — <em>z</em> can change in <strong>three</strong> distinct ways:
        <em>only</em> x changes (y held fixed), <em>only</em> y changes (x held fixed),
        or <em>both</em> x and y change simultaneously. The three dashed projection lines
        from point A to the coordinate planes (xy, xz, yz) make each dependency visible.
        To study the isolated effect of x alone, we hold y constant — this idea of
        <strong>freezing one input</strong> to examine the other is the key intuition.
      </SectionDescription>
      ...
      <FormulaBox>
        ...
        <Formula>
          z = f(x, y) = x² + y²<br/><br/>
          z depends on both x and y. To measure how z responds to
          <em> just one</em> variable, we freeze the other input.<br/><br/>
          At (x₀, y₀) = ({x0.toFixed(1)}, {y0.toFixed(1)}): &nbsp; z₀ = {z0.toFixed(3)}
        </Formula>
      </FormulaBox>
